...
genMethodInvk size ctx ct t m pt = 
  do e <- genExpr (size `div` 2) ct ctx t
     el <- Control.Monad.mapM (genExpr (size `div` 4) ct ctx) pt
...
genExpr size ct ctx t 
  | size > 0 = 
      oneof ([do el <- Control.Monad.mapM (genExpr (size `div` 4) ct ctx) ft
                 return (CreateObject t el)] ++ 
              maybeToList (pickVar ctx (TypeClass t)) ++ 
              fc ++ mc ++ ucc)
  | otherwise = do el <- Control.Monad.mapM (genExpr (size `div` 5) ct ctx) ft
...
-- Objective: Tests if the generated programs are compiled by 'javac'.
-- Params: None.
