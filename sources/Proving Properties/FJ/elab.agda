...
-- We assume a function from Name to Ty

postulate 
  Name⇒Ty : Name → Ty

-- Converting Meth to MSig

Meth⇒MSig : Meth → MSig
Meth⇒MSig MD = mkMSig (Name⇒Ty (Meth.ret MD)) (map (λ p → Name⇒Ty (proj₂ p)) (Meth.params MD))

-- Converting Class to CSig

Class⇒CSig : Class → CSig
Class⇒CSig c = mkCSig (map (λ f → Name⇒Ty (proj₂ f)) (Class.flds c)) (map (λ m → Meth⇒MSig (proj₂ m)) (Class.meths c))
...
elabCT [] = []
elabCT (c ∷ ct) = Class⇒CSig (proj₂ c) ∷ elabCT ct
...
postulate 
  ...
  eq-meths : ∀ {C m MD meths} → method C m MD → meths ∋ m ∶ MD → signatures (Name⇒Ty C) ≡ map (λ m' → Meth⇒MSig (proj₂ m')) meths
...
elabMeth m@(this {cd = record { cname = cname ; flds = _ ; meths = meths }} wtc wtm) rewrite eq-meths m wtm = elabMethList meths wtm
